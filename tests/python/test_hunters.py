...
class TestHunters(TestCase):
    ...
    def test_ast_extractor_call(self):
        ...
        test_call = Call(
            func=Name(id="test"),
            args=[Constant(value="test1"), Constant(value="test2")],
        )
    ...
    def test_ast_extractor_counter(self):
        ...
        test_expr = Expr(
            value=Call(
                func=Attribute(value=Name(id="test_counter"), attr="add"),
                args=[
                    Constant(value=2),
                    Dict(
                        keys=[Constant(value="key1")], values=[Constant(value="value1")]
                    ),
                ],
            )
        )
    ...
    def test_ast_extractor_add_event_simple(self):
        ...
        test_expr = Expr(
            value=Call(
                func=Attribute(
                    value=Name(id="span", ctx=Load()), attr="add_event", ctx=Load()
                ),
                args=[Constant(value="custom event")],
                keywords=[
                    keyword(
                        arg="attributes",
                        value=Dict(
        ...
        expected = {
            "func": "span.add_event",
            "args": ["custom event"],
            "keywords": {"attributes": {"key1": "value1"}},
        }
